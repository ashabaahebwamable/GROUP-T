...
The brief requires data the team can obtain in Week 1 and forbids dependence on confidential databases. Synthetic data is not a compromise in this project: because the system's central difficulty is *inconsistency between quotations*, authoring the corpus is what lets the team place specific, known inconsistencies into it and then test whether the system handles them. A real corpus would be less useful, not more.
...
| Quotation omitting its unit of measure | **[TO BE ADDED]** | Refusal to default a missing field (US-03 criterion 2) |
...
Week 1 delivers a working slice with every trap represented. Expansion is a Week 2-3 task with a named owner **[PLACEHOLDER]**.
